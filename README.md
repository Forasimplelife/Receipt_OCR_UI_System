...
この記事では、OCR認識UIシステムの構築プロセスについて詳しく紹介します。このシステムは、レシート画像から電話番号、日時、金額などの重要な情報を正確に抽出することできます。システム構築には、2つのAIモデル（分類モデルと物体検出モデル）を統合して、レシート上の情報を効果的に解析しています。さらに、文字認識にはEasyOCRを使用し、抽出されたデータの精度向上になります。また、 UIの構築にはStreamlitを使用しており、簡単で画面で操作を実現しました。
