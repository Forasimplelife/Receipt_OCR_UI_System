...
分類モデルはレシート画像の回転角度（0度、90度、180度、270度）を自動的に判別し、どの向きの画像でも正しく処理できるように設計されています。物体検出モデルは、レシート内の店舗名や日時、金額などの情報が記載された領域を効率的に特定し、文字認識に最適なデータを提供します。さらに、文字認識部分にはEasyOCRを採用し、多言語対応で高精度なテキスト抽出を実現しました。

これらのAIモデルを基盤に、Streamlitを用いてシンプルで直感的なUIを開発し、ユーザーが専門知識を必要とせずに簡単に利用できる環境を提供しています。このシステムは、手作業でのデータ入力を大幅に削減し、効率化と正確性の向上を図ることを目的としています。


分類モデルにはResNetを採用し、レシート画像の回転角度（0度、90度、180度、270度）を自動的に分類します。この分類により、画像の向きに関係なく後続の処理を最適化し、高い認識精度を確保しています。モデルの詳細は以下のリポジトリをご参照ください：
Receipt Classification Model

物体検出モデルとしてはYOLOv9を使用し、レシート内の重要な情報が記載された領域（店舗名、日時、金額など）を特定します。効率的な情報抽出を可能にするこのモデルの詳細については、以下のリポジトリをご参照ください：
Receipt Detection Model

文字認識にはEasyOCRを使用し、検出されたテキスト領域から日本語や英語などの多言語文字情報を高精度で抽出します。

また、システム全体のUI設計にはStreamlitを採用し、シンプルかつ使いやすい操作性を実現しています。技術的知識がなくても簡単に利用可能です。

今後、モデルの改良やデータセットの拡充により、さらなる認識精度向上と多様なレシート形式への対応を目指します。
